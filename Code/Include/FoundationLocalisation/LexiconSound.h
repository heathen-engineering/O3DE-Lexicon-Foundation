...
{
    ///<summary>
    /// A localisation-aware sound asset reference for use in components and assets.
    ///
    /// Localised  — m_keyOrValue holds a dot-path key (e.g. "Audio.Theme").
    ///              The key resolves to an AZ::Uuid (sound asset ID) via the
    ///              active LexiconAssemblyAsset. Call GetHash() for bus lookups.
    ///
    /// Literal    — m_literalAssetId holds the sound asset UUID directly.
    ...
    {
    public:
        AZ_TYPE_INFO(LexiconSound, LexiconSoundTypeId)
        AZ_CLASS_ALLOCATOR(LexiconSound, AZ::SystemAllocator, 0)
